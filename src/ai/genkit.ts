...
export async function runWithModelSafe<T>(
  ...
  for (const modelId of MODELS_TO_TRY) {
    ...
    try {
      ...
      console.info(
        `[AI-SUCCESS] Model: ${modelId} | Latency: ${latency}ms | Total: ${totalLatency}ms`,
      );
      ...
      if (msg.includes("404")) category = "MODEL_MISMATCH_OR_UNAVAILABLE";
        ...
        category = "GOOGLE_API_OUTAGE";

      console.warn(
        `[AI-RETRY] ${category} for ${modelId} after ${latency}ms: ${msg}`,
      );
  ...
  console.error(
    `[AI-CRITICAL] Total Latency: ${finalLatency}ms | ${finalErrorMessage}`,
  );
