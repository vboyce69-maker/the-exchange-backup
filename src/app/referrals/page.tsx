...
function ReferralContent() {
  ...
  const copyToClipboard = () => {
    ...
    toast({
      title: "Link Copied!",
      description: "Share with potential sellers to earn rewards.",
    });
  ...
  const milestones = [
    {
      ...
      status:
        (referralData?.invites?.length || 0) >= 1 ? "completed" : "locked",
    },
    {
      target: 3,
      reward: "Free Featured Listing",
      status:
        (referralData?.invites?.length || 0) >= 3 ? "completed" : "locked",
    },
    {
      target: 10,
      reward: "R500 Trade Credit",
      status:
        (referralData?.invites?.length || 0) >= 10 ? "completed" : "locked",
    },
  ];
          ...
          <div className="bg-[#225BC3] rounded-[3.5rem] p-12 text-white relative overflow-hidden shadow-2xl">
            ...
            <div className="relative z-10 space-y-6">
              ...
              <h1 className="text-3xl lg:text-5xl font-black tracking-tighter uppercase leading-none">
                Refer & <span className="text-[#34CBED]">Earn</span>
              </h1>
              <p className="text-lg text-white/80 font-bold max-w-md leading-relaxed">
                Join the movement. Invite sellers to unlock lifetime perks.
              </p>
            </div>
          </div>
            ...
            <Card className="lg:col-span-7 rounded-[3rem] border-none shadow-xl bg-white p-10 ring-1 ring-slate-100 flex flex-col justify-between">
              <div className="space-y-8">
                ...
                <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-[2.5rem] border-2 border-dashed border-slate-200">
                  ...
                  <Button
                    ...
                    className="rounded-2xl bg-[#225BC3] text-white font-black px-8 h-14 hover:scale-105 transition-transform"
                  >
                    {copied ? (
              ...
              <div className="grid grid-cols-3 gap-6 mt-16">
                <div className="p-8 bg-slate-50 rounded-[2.5rem] text-center border border-slate-100">
                  <p className="text-4xl font-black text-[#225BC3]">
