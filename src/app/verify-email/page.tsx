  ...
  const handleRefreshStatus = async () => {
    ...
    try {
      ...
      if (user.emailVerified) {
        ...
        toast({
          title: "Identity Pillar Activated",
          description: "Email verification sync complete.",
        });
        router.push("/");
      } else {
  ...
  const handleSendVerification = async () => {
    ...
    try {
      ...
      if (err.code === "auth/requires-recent-login") {
        errorMessage =
          "Security policy requires a recent login. Please sign out and back in.";
      } else if (err.code === "auth/unauthorized-domain") {
        errorMessage =
          "Domain Unauthorized: Add this domain to Authorized Domains in Firebase Console.";
      } else if (err.code === "auth/too-many-requests") {
        errorMessage =
          "System throttled: Too many emails sent. Please wait a few minutes.";
      }
      ...
      <main className="container mx-auto px-4 py-12 flex justify-center">
        <div className="max-w-md w-full space-y-6">
              ...
              {user.emailVerified ? (
                <div className="space-y-4 animate-in zoom-in-95">
                  ...
                  <p className="text-slate-500 text-sm font-medium">
                    Your identity is confirmed. The marketplace is now fully
                    unlocked.
                  </p>
                  <Button
                    className="w-full h-16 rounded-2xl bg-[#225BC3] text-white font-black shadow-xl text-lg"
                    onClick={() => router.push("/")}
                  >
                  ...
                  <div className="space-y-4 text-left">
                    {!user.email && !isEditing && (
                      <Alert className="bg-orange-50 border-orange-200 rounded-2xl border">
                        ...
                        <AlertTitle className="font-black text-[10px] uppercase tracking-widest text-orange-800">
                          Registration Incomplete
                        </AlertTitle>
                        <AlertDescription className="text-xs font-bold text-orange-700 leading-tight">
                          Phone-only users must register an email to enable
                          trade protection.
                        </AlertDescription>
                      </Alert>
                    )}
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        if (!user.emailVerified)
                          toast({
                            title: "Checking...",
                            description: "Identity sync in progress.",
                          });
                      }}
                  ...
                  >
                    ...
                    >
                      <AccordionTrigger className="text-[10px] font-black uppercase text-slate-400 py-2 hover:no-underline hover:text-[#225BC3] transition-colors">
                        <Info className="w-3 h-3 mr-2" /> Help: Link not
                        arriving?
                      </AccordionTrigger>
                      <AccordionContent className="space-y-4 pt-2">
                        <div className="p-4 bg-slate-50 rounded-2xl space-y-2 border border-slate-100">
                          ...
                          <p className="text-[9px] text-slate-500">
                            Automated links are often filtered. Look for "The
                            Exchange Identity".
                          </p>
                        </div>
                        <div className="p-4 bg-slate-50 rounded-2xl space-y-2 border border-slate-100">
                          ...
                          <p className="text-[9px] text-slate-500 leading-relaxed">
                            Developers: Ensure the current domain is in the{" "}
                          ...
                          <Button
                            variant="link"
                            className="p-0 h-auto text-[9px] font-black text-[#225BC3] uppercase"
                            asChild
                          >
                            <a
          ...
          <div className="p-6 bg-blue-50/50 rounded-[2rem] border border-blue-100 flex gap-4">
            ...
            <div>
              <p className="text-[10px] font-black text-[#225BC3] uppercase tracking-widest">
                Platform Integrity
              </p>
              <p className="text-[9px] text-blue-600 font-bold leading-relaxed mt-1">
                Once verified, your marketplace identity is permanent. This
                activation persists across all devices and sessions.
              </p>
            </div>
          </div>
        </div>
