...
import {
  ...
  SellerDemandInsightsOutput,
} from "@/ai/flows/seller-demand-insights";
import {
  Card,
  CardContent,
  CardHeader,
...
export default function InsightsPage() {
  ...
  const [insights, setInsights] = useState<SellerDemandInsightsOutput | null>(
    null,
  );
  ...
  const handleBoostListings = async () => {
    if (!user || !db || !userListings) {
      toast({
        ...
        description:
          "Please sign in and ensure you have active listings to boost.",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description:
          "All your active items are now prioritized in search results.",
      });
    } catch (err) {
      toast({
        variant: "destructive",
        title: "Boost Error",
        description: "Could not apply premium status.",
      });
    } finally {
      setIsBoosting(false);
    }
  ...
  const handleViewVolumeDetails = () => {
    toast({
      ...
      description:
        "Calculating historical volume trends and competitive density for these categories...",
    });
  };
        ...
        <div className="flex flex-col lg:flex-row items-start justify-between gap-8 mb-12">
          <div className="max-w-2xl space-y-4">
            <div className="flex items-center gap-3">
              ...
              {isCached && (
                <Badge
                  variant="outline"
                  className="border-slate-200 text-slate-400 font-bold text-[9px] uppercase"
                >
            ...
            <p className="text-lg text-slate-600 font-medium leading-relaxed">
              We analyze regional search data and supply volume to help you
              price and position your listings for the fastest possible sale.
            </p>
          </div>
          <div className="flex gap-4">
            <Button
              ...
              className="rounded-2xl h-12 font-black uppercase text-[10px] tracking-widest"
            >
              Refresh Data
            </Button>
            <Button
          ...
          <Card className="lg:col-span-2 rounded-[3rem] border-none shadow-2xl bg-white overflow-hidden">
            <CardHeader className="bg-slate-50 p-8 border-b border-slate-100">
              ...
              <CardDescription className="font-bold text-slate-500">
                Areas with peak buyer activity this week
              </CardDescription>
            </CardHeader>
            <CardContent className="p-8 space-y-8">
              <div className="relative aspect-video bg-blue-50 rounded-[2rem] overflow-hidden flex items-center justify-center border-2 border-dashed border-[#225BC3]/10">
                <div className="text-center p-8 relative z-10">
                  ...
                  <p className="text-[#225BC3] font-black uppercase text-xs tracking-widest">
                    Interactive Supply Heatmap
                  </p>
                  <p className="text-[10px] text-slate-400 font-bold mt-2">
                    Target listings to these high-conversion zones
                  </p>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {insights?.highDemandAreas.map((area, i) => (
                  <div
                    key={i}
                    className="p-6 border border-slate-100 rounded-3xl bg-white shadow-sm flex flex-col gap-3 group hover:ring-2 hover:ring-[#225BC3]/5 transition-all"
                  >
          ...
          <Card className="rounded-[3rem] border-none shadow-2xl bg-white overflow-hidden">
            ...
            <CardContent className="p-8 space-y-6">
              {insights?.optimalListingCategories.map((cat, i) => (
                <div
                  key={i}
                  className="p-5 border border-slate-50 rounded-2xl bg-slate-50/50 flex flex-col gap-3 group hover:bg-white hover:shadow-lg transition-all"
                >
                  <div className="flex justify-between items-center">
                    <span className="font-black text-slate-900 text-xs uppercase group-hover:text-[#225BC3] transition-colors">
                      {cat.categoryName}
                    </span>
