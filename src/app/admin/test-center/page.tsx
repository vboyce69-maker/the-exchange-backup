...
import {
  ...
  AutonomousTesterOutput,
} from "@/ai/flows/autonomous-tester";
...
export default function AutonomousTestCenter() {
  ...
  const [intensity, setIntensity] = useState<"NORMAL" | "HIGH" | "EXTREME">(
    "NORMAL",
  );
  ...
  const runAllTests = async (suiteId?: string) => {
    ...
    try {
      ...
      toast({
        variant: "destructive",
        title: "Audit Error",
        description: "The agent encountered a system timeout.",
      });
    } finally {
      setIsRunning(false);
    }
        ...
        <div className="max-w-6xl mx-auto space-y-8">
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-8 bg-[#225BC3] p-8 lg:p-10 rounded-[2.5rem] shadow-2xl relative overflow-hidden text-white">
            ...
            <div className="relative z-10 space-y-4 text-center lg:text-left flex-1">
              ...
              <p className="text-white/80 font-bold text-sm max-w-lg leading-relaxed italic">
                "Breaking the platform to build a fortress. Stress testing 100k
                concurrent users and adversarial security flows."
              </p>
                ...
                {["NORMAL", "HIGH", "EXTREME"].map((lvl) => (
                  <button
                    ...
                    className={cn(
                      ...
                      intensity === lvl
                        ? "bg-white text-[#225BC3] shadow-lg"
                        : "bg-white/10 text-white hover:bg-white/20",
                    )}
                  >
                    {lvl === "EXTREME" && (
                      <Flame className="w-2.5 h-2.5 mr-1 inline-block animate-pulse" />
                    )}
                    {lvl}
                  </button>
                ))}
              ...
              <Button
                className={cn(
                  ...
                  intensity === "EXTREME"
                    ...
                    : "bg-white text-[#225BC3] border-white/20",
                )}
                onClick={() => runAllTests()}
                disabled={isRunning}
              >
          ...
          {results && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in zoom-in-95">
              <Card className="rounded-[2.5rem] border-none shadow-xl bg-white p-8 flex flex-col items-center text-center gap-4">
                <div className="p-5 bg-blue-50 rounded-[2rem] text-[#225BC3]">
                ...
                <div>
                  ...
                  <p className="text-4xl font-black text-slate-900">
                    {results.scalabilityRating}/10
                  </p>
                </div>
              </Card>
              <Card className="rounded-[2.5rem] border-none shadow-xl bg-white p-8 flex flex-col items-center text-center gap-4">
                <div className="p-5 bg-orange-50 rounded-[2rem] text-[#FF8C00]">
              ...
              <Card className="rounded-[2.5rem] border-none shadow-xl bg-white p-8 flex flex-col items-center text-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xl font-black text-slate-900 truncate max-w-[200px]">
                    {results.mostUnstableFeature}
                  </p>
                </div>
              </Card>
            </div>
          ...
          {results && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-6">
              <div className="flex items-center justify-between">
                ...
                <Badge
                  variant="outline"
                  className="font-black text-[10px] px-4 py-1.5 rounded-full border-slate-200"
                >
                  Audit Date:{" "}
                  {new Date(results.auditTimestamp).toLocaleString()}
                </Badge>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {results.reports.map((report, i) => (
                  <Card
                    key={i}
                    className="rounded-[3rem] border-none shadow-2xl bg-white overflow-hidden ring-1 ring-slate-100"
                  >
                    <div
                      className={cn(
                        "p-8 flex items-center justify-between",
                        report.crash_risk_level === "CRITICAL" ||
                          report.crash_risk_level === "HIGH"
                    ...
                    >
                      <div className="space-y-1">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-widest opacity-70">
                          {report.passed}/{report.total_tests} Tests Passed •{" "}
                          {report.category}
                        </p>
                      </div>
                      <Badge
                        className={cn(
                          ...
                          report.crash_risk_level === "CRITICAL"
                            ...
                            : report.crash_risk_level === "HIGH"
                              ...
                              : report.crash_risk_level === "MEDIUM"
                      ...
                      {report.performance && (
                        <div className="grid grid-cols-2 gap-4">
                          <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-1">
                            ...
                            <p className="text-xl font-black text-[#225BC3]">
                              {report.performance.avgResponseTimeMs}ms
                            </p>
                          </div>
                          <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-1">
                            <p className="text-[8px] font-black text-slate-400 uppercase">
                            ...
                            <p className="text-xl font-black text-[#FF8C00]">
                              {report.performance.peakLatencyMs}ms
                            </p>
                          </div>
                        </div>
                      )}
                      ...
                      {report.critical_bugs.length > 0 && (
                        <div className="space-y-3">
                          ...
                          {report.critical_bugs.map((bug, bi) => (
                            <div
                              key={bi}
                              className="p-4 bg-red-50 rounded-2xl border border-red-100 text-red-800 text-xs font-bold leading-relaxed"
                            >
                              🚨 {bug}
                            </div>
                          ))}
              ...
              <Card className="rounded-[3.5rem] border-none shadow-2xl bg-white p-12 overflow-hidden relative">
                ...
                <div className="relative z-10 space-y-6">
                  <h3 className="text-3xl font-black text-[#225BC3] uppercase tracking-tighter">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {results.topFailurePoints.map((point, pi) => (
                      <Badge
                        key={pi}
                        className="bg-red-100 text-red-700 border-none px-4 py-1 font-black text-[10px] uppercase"
                      >
                        {point}
                      </Badge>
                    ))}
          ...
          {!results && !isRunning && (
            <div className="text-center py-32 bg-white rounded-[4rem] shadow-sm border-2 border-dashed border-slate-100">
              ...
              <p className="font-black text-[#225BC3] uppercase tracking-widest text-lg">
                Initialize Security Hub
              </p>
              <p className="text-muted-foreground text-sm font-medium mt-2">
                Select an intensity level and run the full suite audit.
              </p>
            </div>
          )}
        </div>
