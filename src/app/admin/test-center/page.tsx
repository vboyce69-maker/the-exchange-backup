...
export default function AutonomousTestCenter() {
  ...
  const runAllTests = async (suiteId?: string) => {
    setIsRunning(true);
    setResults(null);
    setCurrentStep(suiteId ? `Auditing ${suiteId}...` : "Performing Full Platform Audit...");
    ...
    try {
      ...
      toast({
        ...
        description: suiteId ? `${suiteId} module verified.` : "Global platform integrity scan finished.",
      });
    } catch (err: any) {
      console.error(err);
      toast({ variant: "destructive", title: "Audit Error", description: "The diagnostic agent encountered a timeout." });
    } finally {
      setIsRunning(false);
      setCurrentStep("");
    }
        ...
        <div className="max-w-6xl mx-auto space-y-12">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-8 bg-white p-10 lg:p-12 rounded-[3.5rem] shadow-2xl border border-[#225BC3]/5 relative overflow-hidden">
            ...
            <div className="relative z-10 space-y-4">
              ...
              <p className="text-muted-foreground font-medium text-lg max-w-md italic">"Simulating real interactions, stress-testing core systems, and flagging vulnerabilities."</p>
            </div>
            ...
            <div className="relative z-10 flex flex-col gap-4 w-full md:w-auto">
              <Button 
                ...
                disabled={isRunning}
              >
                {isRunning ? <Loader2 className="w-8 h-8 animate-spin" /> : "Run Full Platform Audit"}
              </Button>
              {isRunning && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <Card className="rounded-[2.5rem] border-none shadow-xl bg-white p-6 space-y-4 group hover:ring-2 hover:ring-[#225BC3]/5 transition-all cursor-pointer" onClick={() => runAllTests("KYC")}>
               ...
               <p className="text-lg font-black text-slate-900">Biometric Audit</p>
            </Card>
            <Card className="rounded-[2.5rem] border-none shadow-xl bg-white p-6 space-y-4 group hover:ring-2 hover:ring-[#225BC3]/5 transition-all cursor-pointer" onClick={() => runAllTests("Scam")}>
               ...
               <h3 className="font-black text-xs uppercase tracking-widest text-slate-400">Scam Detection</h3>
               <p className="text-lg font-black text-slate-900">NLP & Pattern Scan</p>
            </Card>
            <Card className="rounded-[2.5rem] border-none shadow-xl bg-white p-6 space-y-4 group hover:ring-2 hover:ring-[#225BC3]/5 transition-all cursor-pointer" onClick={() => runAllTests("Location")}>
               ...
               <p className="text-lg font-black text-slate-900">GPS & Spoofing</p>
            </Card>
            <Card className="rounded-[2.5rem] border-none shadow-xl bg-white p-6 space-y-4 group hover:ring-2 hover:ring-[#225BC3]/5 transition-all cursor-pointer" onClick={() => runAllTests("Auth")}>
               <div className="p-4 bg-purple-100 rounded-2xl w-fit text-purple-600"><Lock className="w-6 h-6" /></div>
               <h3 className="font-black text-xs uppercase tracking-widest text-slate-400">Session Security</h3>
               <p className="text-lg font-black text-slate-900">Brute Force / MFA</p>
            </Card>
          </div>
          ...
          {!results && !isRunning && (
            <div className="text-center py-24 bg-white rounded-[4rem] shadow-sm border-2 border-dashed border-slate-100">
               ...
               <p className="text-muted-foreground text-sm font-medium mt-2">Run targeted stress tests or a full platform resilience scan.</p>
            </div>
          )}
        </div>
      </main>
