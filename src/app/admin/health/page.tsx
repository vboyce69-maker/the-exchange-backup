...
import {
  resolveSystemError,
  ErrorResolverOutput,
} from "@/ai/flows/error-resolver-flow";
import {
  Activity,
  ShieldCheck,
  AlertCircle,
  ...
  const handleAnalyzeError = async (error: any) => {
    ...
    try {
      ...
      toast({
        ...
        description: "Could not reach resolution engine.",
      });
    } finally {
      setAnalyzingId(null);
    }
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="space-y-1">
              ...
              <p className="text-muted-foreground font-black uppercase text-[10px] tracking-widest flex items-center gap-2">
                <Activity className="w-3 h-3 text-[#34CBED]" /> Real-time
                Diagnostics & AI Correction
              </p>
            </div>
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="text-2xl font-black text-slate-900 tracking-tight uppercase">
                Recent Stability Logs
              </h2>
              <Button
                variant="ghost"
                className="rounded-xl font-bold text-[#225BC3]"
            ...
            <div className="space-y-4">
              {isLoading || !db ? (
                <div className="py-20 text-center">
                  <RefreshCw className="w-10 h-10 animate-spin text-slate-200 mx-auto" />
                </div>
              ) : errors && errors.length > 0 ? (
                errors.map((error) => (
                  <Card
                    key={error.id}
                    className="rounded-[2.5rem] border-none shadow-xl bg-white overflow-hidden ring-1 ring-slate-100"
                  >
                        ...
                        <div className="lg:w-1/3 bg-[#225BC3]/5 p-6 rounded-3xl border border-[#225BC3]/10 space-y-4 animate-in fade-in slide-in-from-right-4">
                          ...
                          <div className="space-y-3">
                            <p className="text-[11px] font-bold text-slate-700">
                              {error.aiResolution.explanation}
                            </p>
                ...
                <div className="text-center py-24 bg-white rounded-[3rem] shadow-sm border-2 border-dashed border-slate-100">
                  ...
                  <p className="text-muted-foreground text-sm font-medium">
                    Application integrity is currently optimal.
                  </p>
                </div>
              )}
            </div>
