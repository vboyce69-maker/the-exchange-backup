...
export function LiveMeetupTracker({ buyerName, sellerName, safeZoneName, onArrival }: TrackerProps) {
      ...
      <Card className="rounded-[2rem] border-none shadow-2xl bg-white overflow-hidden ring-1 ring-slate-100">
        <div className="bg-[#225BC3] p-4 text-white flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
               <p className="text-[7px] font-black uppercase tracking-widest opacity-70 leading-none">Meetup Point</p>
               <h4 className="text-xs font-black uppercase">{safeZoneName}</h4>
            </div>
          </div>
          <Badge className="bg-[#34CBED] text-white border-none text-[8px] font-black px-2 py-0.5">Live Tracking</Badge>
        </div>
        ...
        <div className="p-5 space-y-6">
          ...
          <div className="relative h-2 bg-slate-100 rounded-full flex items-center">
             ...
             <div
              ...
              style={{ left: `${Math.max(0, 100 - (buyerDist / 5) * 100)}%`, transform: 'translateX(-50%)' }}
             >
                ...
                <span className="text-[6px] font-black uppercase text-blue-600 bg-white px-1 whitespace-nowrap">You</span>
             </div>

             {/* Seller Marker */}
             <div
              className="absolute transition-all duration-1000 ease-linear flex flex-col items-center"
              style={{ left: `${Math.max(0, 100 - (sellerDist / 5) * 100)}%`, transform: 'translateX(-50%)' }}
             >
                ...
                <span className="text-[6px] font-black uppercase text-[#FF8C00] bg-white px-1 whitespace-nowrap">{sellerName}</span>
             </div>
          ...
          <div className="grid grid-cols-2 gap-4">
             <div className={cn(
               ...
               isBuyerArrived ? "bg-green-50 border-green-200" : "bg-blue-50 border-blue-100"
             )}>
                <div className="flex justify-between items-center mb-2">
                   <span className="text-[8px] font-black uppercase text-slate-400">Your Status</span>
                   {isBuyerArrived && <CheckCircle2 className="w-3 h-3 text-green-500" />}
                </div>
                <p className="text-sm font-black text-slate-900 leading-none">
                  {isBuyerArrived ? "Arrived" : `${buyerDist}km away`}
                </p>
                <p className="text-[9px] font-bold text-slate-500 mt-1">ETA: {Math.ceil(buyerDist * 2)} mins</p>
             </div>

             <div className={cn(
               "p-4 rounded-2xl border transition-all",
               isSellerArrived ? "bg-green-50 border-green-200" : "bg-orange-50 border-orange-100"
             )}>
                <div className="flex justify-between items-center mb-2">
                   <span className="text-[8px] font-black uppercase text-slate-400">Seller Status</span>
                   {isSellerArrived && <CheckCircle2 className="w-3 h-3 text-green-500" />}
                </div>
                <p className="text-sm font-black text-slate-900 leading-none">
                  {isSellerArrived ? "Arrived" : `${sellerDist}km away`}
                </p>
                <p className="text-[9px] font-bold text-slate-500 mt-1">ETA: {Math.ceil(sellerDist * 1.5)} mins</p>
             </div>
          </div>
          ...
          {isBuyerArrived && isSellerArrived ? (
            <div className="p-4 bg-green-500 rounded-2xl text-center space-y-2 shadow-xl animate-bounce">
               <div className="flex items-center justify-center gap-2 text-white">
                  ...
                  <span className="text-xs font-black uppercase tracking-tighter">Both Parties Arrived</span>
               </div>
               <p className="text-[9px] text-white/90 font-bold">You are at the Safe Zone. Proceed with inspection.</p>
            </div>
          ) : (
            <div className="flex items-center justify-center gap-2 opacity-50">
               <Clock className="w-3 h-3 text-slate-400" />
               <span className="text-[8px] font-black uppercase tracking-widest text-slate-400">Monitoring safe arrival...</span>
            </div>
          )}
        </div>
      </Card>
