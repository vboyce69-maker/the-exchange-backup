  ...
  if (!isLoaded) {
    return (
      <div className="w-full h-[600px] bg-slate-50 rounded-[2.5rem] flex flex-col items-center justify-center border-2 border-dashed border-slate-200">
        ...
        <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Loading Map Engine...</p>
      </div>
    );
  }
  ...
  if (!process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY) {
    return (
      <div className="w-full h-[600px] bg-blue-50/50 rounded-[2.5rem] p-12 flex flex-col items-center justify-center text-center space-y-6 relative overflow-hidden ring-1 ring-blue-100">
         <div className="absolute top-0 right-0 p-8 opacity-10"><MapPin className="w-32 h-32 text-primary" /></div>
         <div className="w-24 h-24 bg-white rounded-3xl flex items-center justify-center shadow-xl mb-4">
            <Zap className="w-12 h-12 text-primary animate-pulse" />
         </div>
         <div className="max-w-md space-y-3">
            <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">Geographic Discovery Interface</h3>
            <p className="text-sm text-slate-500 font-medium">To activate the live listing map, please configure a Google Maps API Key in your environment settings.</p>
         ...
         <div className="grid grid-cols-3 gap-3 w-full max-w-sm mt-8">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="h-2 bg-blue-100 rounded-full animate-pulse" />
            ))}
         </div>
      </div>
    );
    ...
    <div className="relative shadow-2xl rounded-[2.5rem] overflow-hidden ring-1 ring-slate-100">
      ...
      >
        {listings.map((listing) => {
          ...
          const lat = listing.latitude || -26.2041 + (Math.random() - 0.5) * 0.1;
          const lng = listing.longitude || 28.0473 + (Math.random() - 0.5) * 0.1;
          ...
          >
            <Card className="w-56 rounded-2xl border-none shadow-none p-0 overflow-hidden">
               <div className="relative aspect-video w-full">
                  <Image 
                    src={selectedListing.imageUrls?.[0] || "https://picsum.photos/seed/map/400/200"} 
               ...
               <div className="p-4 space-y-2">
                  <div className="flex justify-between items-start gap-2">
                    <h4 className="font-black text-[10px] uppercase text-slate-900 truncate leading-tight flex-1">{selectedListing.title}</h4>
                    {selectedListing.isBoosted && <Zap className="w-3.5 h-3.5 text-accent fill-current" />}
                  </div>
                  <p className="font-black text-sm text-primary leading-none">R {selectedListing.price?.toLocaleString()}</p>
                  <Link href={`/listings/${selectedListing.id}`}>
                    <Button size="sm" className="w-full h-8 rounded-xl bg-slate-900 text-white font-black text-[8px] uppercase tracking-widest mt-2">
                      View Details
                    </Button>
                  </Link>
               </div>
      ...
      <div className="absolute bottom-6 left-6 right-6 lg:left-auto lg:w-72 bg-white/90 backdrop-blur-md p-6 rounded-3xl shadow-2xl ring-1 ring-slate-100 animate-in slide-in-from-bottom-4">
         <div className="flex items-center gap-3 text-primary mb-3">
            ...
            <span className="text-[9px] font-black uppercase tracking-widest">Regional Discovery</span>
         </div>
         <p className="text-[10px] text-slate-600 font-bold leading-relaxed mb-4">
           Showing {listings.length} verified listings in your current view. Use the map to find deals in secure meetup zones.
         </p>
         <div className="flex items-center gap-2">
            <div className="w-2 h-2 bg-primary rounded-full" />
            <span className="text-[8px] font-black text-slate-400 uppercase">Standard Listing</span>
            <div className="w-2 h-2 bg-accent rounded-full ml-2" />
            <span className="text-[8px] font-black text-slate-400 uppercase">Featured</span>
         </div>
      </div>
    </div>
  );
