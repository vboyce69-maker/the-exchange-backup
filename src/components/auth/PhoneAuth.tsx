  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    if (!formattedPhone.startsWith("+")) {
      toast({
        ...
        description: "Include your country code (e.g. +27).",
      });
      return;
    }
...
if (!auth) return;
      ...
      if (err.code === "auth/unauthorized-domain") {
        errorMessage =
          "Domain Unauthorized: Please add the current preview URL to 'Authorized Domains' in the Firebase Console.";
      } else if (err.code === "auth/operation-not-allowed") {
        errorMessage =
          "Phone Authentication is not enabled in the Firebase Console (Authentication > Sign-in method).";
      } else if (err.code === "auth/too-many-requests") {
        errorMessage =
          "We have sent too many codes to this number. Please try again later.";
      }
    ...
    <div className="w-full">
      {step === "phone" ? (
        ...
        <form onSubmit={handleVerifyOtp} className="space-y-6">
          ...
          <Button
            className="w-full h-16 rounded-2xl bg-[#34CBED] text-white font-black text-lg shadow-xl"
            disabled={loading}
          >
            {loading ? (
              <Loader2 className="w-6 h-6 animate-spin" />
